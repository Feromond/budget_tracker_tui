...
pub fn get_help_for_mode(mode: AppMode) -> Vec<KeyBindingInfo> {
    match mode {
        AppMode::Normal => vec![
            ...
            KeyBindingInfo::new("o", "Settings", "Actions", Some("Open application settings: database location, category management, CSV import/export, budget and display preferences.")),
        ...
        AppMode::Adding | AppMode::Editing => vec![
            ...
            KeyBindingInfo::new("←/→", "Toggle type / Adjust date / Move cursor", "Input", Some("On the Type field toggles Income/Expense; on the Date field moves the date by one day; on text fields moves the cursor.")),
            KeyBindingInfo::new("Shift+←/→", "Jump month (Date field)", "Input", Some("In date fields, moves date by one month instead of one day.")),
            KeyBindingInfo::new("+/-", "Adjust date", "Input", Some("On the Date field, '+' or '=' moves the date forward a day and '-' moves it back.")),
            KeyBindingInfo::new("Enter", "Save / Toggle type / Open selection", "Actions", Some("On the Type field toggles Income/Expense; on Category/Subcategory opens a selection list; on any other field saves the transaction.")),
        ...
        AppMode::AdvancedFiltering => vec![
            ...
            KeyBindingInfo::new("←/→", "Adjust date / Toggle type / Move cursor", "Input", Some("On the Start/End Date fields adjusts by a day; on the Type field toggles Expense/Income; on text fields moves the cursor.")),
            KeyBindingInfo::new("Shift+←/→", "Jump month (date fields)", "Input", None),
            KeyBindingInfo::new("Ctrl+R", "Clear all filters", "Actions", None),
            KeyBindingInfo::new("Enter", "Save & Apply / Open selection", "Actions", Some("On the Category/Subcategory fields opens a selection list; on any other field saves and applies the filters.")),
        ...
        AppMode::Settings => vec![
            ...
            KeyBindingInfo::new("Database Path", "SQLite Database Location", "Fields", Some("Path to the SQLite database that stores your transactions and category catalog. Point this at a new file to create a fresh database, or an existing file to load different data. Tip: place it in iCloud Drive, Google Drive, or Dropbox to sync across devices.")),
            ...
            KeyBindingInfo::new("Import Transactions", "Import from CSV", "Fields", Some("Press Enter to open a path prompt and import a CSV. New rows are added and exact duplicates are skipped; recurring occurrences are regenerated automatically.")),
            KeyBindingInfo::new("Export Transactions", "Export to CSV", "Fields", Some("Press Enter to open a path prompt and export all transactions to a CSV file you can back up or share.")),
            ...
            KeyBindingInfo::new("←/→", "Toggle Options / Move cursor", "Navigation", Some("On toggle settings changes the value (e.g. Yes/No); on path/number fields moves the text cursor.")),
            KeyBindingInfo::new("Ctrl+D", "Reset Database Path", "Actions", Some("On the Database Path field, resets it to the default location for the current data directory.")),
            KeyBindingInfo::new("Ctrl+U", "Clear Field", "Actions", None),
            KeyBindingInfo::new("Enter", "Save Settings / Activate Action", "Actions", Some("On editable fields saves your settings; on action rows (Manage Categories, Import, Export) opens that action.")),
        ...
        AppMode::RecurringSettings => vec![
            ...
            KeyBindingInfo::new("←/→", "Toggle active / Adjust end date", "Input", Some("On the Active field toggles recurrence on/off; on the End Date field moves the date by one day.")),
            KeyBindingInfo::new("Shift+←/→", "Jump month (End Date)", "Input", None),
            KeyBindingInfo::new("Enter", "Select frequency / Save", "Actions", Some("On the Frequency field opens the frequency picker; on any other field saves the recurring settings.")),
            KeyBindingInfo::new("Esc", "Cancel", "Actions", None),
        ...
        AppMode::ImportTransactions | AppMode::ExportTransactions => vec![
            KeyBindingInfo::new("Any Char", "Type the file path", "Input", Some("Type or paste an absolute path to the CSV file.")),
            KeyBindingInfo::new("←/→", "Move cursor", "Navigation", None),
